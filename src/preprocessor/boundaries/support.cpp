...
Support::Support(int index, std::optional<double> disp_x, std::optional<double> disp_y) : index(index), disp_x(disp_y), disp_y(disp_y){};
...
Support Support::createSupport(std::unordered_map<std::string, std::optional<double>> support_components) {
    ...
    return Support(support_components["index"].value(), support_components["disp_x"], support_components["disp_y"]);
}
