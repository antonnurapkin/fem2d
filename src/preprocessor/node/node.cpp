...
std::shared_ptr<Node> Node::createNode(std::unordered_map<std::string, std::optional<double>> node_data) {
    ...
    return std::make_shared<Node>(node_data["index"].value(), node_data["x"].value(), node_data["y"].value());
}
...
void Node::setDispX(double disp_x) {
    this->disp_x = disp_x;
}

void Node::setDispY(double disp_y) {
    this->disp_y = disp_y;
}
...
double Node::getDispX() const {
    return this->disp_x;
}

double Node::getDispY() const {
    return this->disp_y;
}
